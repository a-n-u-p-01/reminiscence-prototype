...
export default function ConceptDetail({ concept, onBack, onSave, onDelete }) {
  ...
  // Bulletproof clean node auto-resizer executed precisely at mount phase
  ...
  // Synchronous height tracking on every single keystroke event
  ...
  return (
    <div className="w-full max-w-3xl mx-auto pb-20 relative antialiased select-text selection:bg-zinc-800 selection:text-white">
      ...
      <div className={`w-full transition-all duration-200 ${isAnyModalOpen ? 'scale-[0.99] opacity-30 pointer-events-none select-none filter blur-[0.5px]' : 'scale-100 opacity-100'}`}>
        ...
        {/* Navigation Toolbar Controls Container */}
        <div className="border-b border-zinc-800/60 pb-5 space-y-4">
          
          {/* Top Row: Lock Title Left, Lock Save Changes Absolute Right */}
          <div className="flex items-center justify-between gap-4 w-full min-h-[40px]">
            <div className="flex items-center gap-2.5 min-w-0 flex-1">
              ...
              <h1 className="font-semibold tracking-tight text-zinc-100 text-s4 truncate leading-snug">
                {concept.conceptName || concept.name}
              </h1>
            </div>
            
            {/* Pinned directly opposite to the title row layout block */}
          ...
          {/* Bottom Row: Independent row for utility groupings underneath */}
        ...
        <div className="mt-8 space-y-6">
          ...
          <div className={`p-4 rounded-xl border transition-all duration-200 ${activeInlineField === 'question' ? 'bg-zinc-950 border-zinc-700 ring-1 ring-zinc-800/50' : 'bg-zinc-900/20 border-zinc-800/50 hover:border-zinc-800/80'}`}>
            ...
            {activeInlineField === 'question' ? (
              <div className="space-y-3">
                <textarea ref={autoResizeRef} rows={1} value={inlineValue} onChange={handleTextareaChange} className="w-full bg-transparent text-s3 text-zinc-200 font-sans leading-relaxed border-none outline-none focus:ring-0 p-0 resize-none overflow-hidden whitespace-pre-wrap min-h-0 h-auto" autoFocus />
          ...
          <div className={`p-4 rounded-xl border transition-all duration-200 ${activeInlineField === 'mainNote' ? 'bg-zinc-950 border-zinc-700 ring-1 ring-zinc-800/50' : 'bg-zinc-900/20 border-zinc-800/50 hover:border-zinc-800/80'}`}>
            ...
            {activeInlineField === 'mainNote' ? (
              <div className="space-y-3">
                <textarea ref={autoResizeRef} rows={1} value={inlineValue} onChange={handleTextareaChange} className="w-full bg-transparent text-s3 text-zinc-300 font-sans leading-relaxed border-none outline-none focus:ring-0 p-0 resize-none overflow-hidden whitespace-pre-wrap min-h-0 h-auto" autoFocus />
                <div className="flex justify-end gap-2 pt-2 border-t border-zinc-900">
                  <button onClick={() => setActiveInlineField(null)} className="text-zinc-500 hover:text-zinc-400 p-1 rounded-lg transition-colors cursor-pointer"><X size={14} /></button>
            ...
            {activeInlineField === 'extraNote' ? (
              <div className="space-y-3">
                <textarea 
                  ...
                  className="w-full bg-transparent text-s3 text-zinc-300 font-sans leading-relaxed border-none outline-none focus:ring-0 p-0 resize-none overflow-hidden whitespace-pre-wrap pt-1 block style-normal min-h-0 h-auto" 
                  autoFocus 
                />
                <div className="flex justify-end gap-2 pt-2 border-t border-zinc-900">
                  <button onClick={() => setActiveInlineField(null)} className="text-zinc-500 hover:text-zinc-400 p-1 rounded-lg transition-colors cursor-pointer"><X size={14} /></button>
